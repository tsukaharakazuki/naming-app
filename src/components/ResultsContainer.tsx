...
export default function ResultsContainer({ surnameA, surnameB, candidatesA, candidatesB, selectedNames, onToggleSelect }: Props) {
  const [activeTab, setActiveTab] = useState<"a" | "b">(candidatesA.length > 0 ? "a" : "b");
  const hasA = candidatesA.length > 0;
  const hasB = candidatesB.length > 0;
  const hasBoth = hasA && hasB;

  const bothGoodNames = useMemo(() => {
    if (!hasBoth) return new Set<string>();
    const threshold = 16;
  ...
  }, [candidatesA, candidatesB, hasBoth]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Desktop: side by side or single */}
      ...
      {/* Mobile: tabs if both, single if one */}
